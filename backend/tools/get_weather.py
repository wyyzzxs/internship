"""get_weather 工具 - mock 实现:读 data/weather_cache.json 兜底。

B 后续替换为和风天气 API + cache 兜底(项目方案 §3.3)。
"""
...
def _get_weather_impl(city: str, start_date: str, days: int = 3) -> dict:
    ...
    return {"city": city, "weather": out, "source": "cache"}


@tool
def get_weather(city: str, start_date: str, days: int = 3) -> str:
    """查询未来 N 天天气预报(B 后续接入和风天气 API,本轮 mock)。
